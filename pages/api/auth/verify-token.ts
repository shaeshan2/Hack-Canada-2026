...
export default async function handler(
  ...
  try {
    ...
    if (bearerToken) {
      if (!issuer) {
        res
          .status(500)
          .json({
            error: "Missing Auth0 issuer configuration",
            authenticated: false,
            role: null,
            userId: null,
          });
      ...
      const audience =
        process.env.AUTH0_AUDIENCE ?? process.env.AUTH0_CLIENT_ID;
      if (!audience) {
        res
          .status(500)
          .json({
            error: "Missing Auth0 audience/client configuration",
            authenticated: false,
            role: null,
            userId: null,
          });
    ...
    res
      .status(200)
      .json({ authenticated: true, userId: dbUser.id, role: dbUser.role });
  } catch (err) {
