...
function AdminReviewPage({ adminName }: Props) {
  ...
  const [flagStatusFilter, setFlagStatusFilter] = useState<FlagStatusFilter>(
    FraudFlagStatus.PENDING_REVIEW,
  );
  ...
  const filteredSellers = useMemo(
    () =>
      pendingSellers.filter(
        (submission) => submission.status === sellerStatusFilter,
      ),
  ...
  async function bulkApprovePendingSellers() {
    ...
    for (const submission of targets) {
      try {
        const res = await fetch(`/api/admin/review/sellers/${submission.id}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
                  ...
                  {selectedSubmission ? (
                    <div className="ar-card">
                      ...
                      <div className="ar-card-body">
                        ...
                        {(selectedSubmission.aiAnalysis || selectedSubmission.aiConfidence !== null) && (
                          <div className="ar-ai-analysis" style={{ marginTop: "16px", padding: "16px", backgroundColor: "var(--bg-glass)", border: "1px solid var(--border-glass)", borderRadius: "var(--radius-md)" }}>
                            <h4 style={{ margin: 0, marginBottom: "8px", fontSize: "14px", color: "var(--text-primary)", display: "flex", alignItems: "center", gap: "6px" }}>
                              <span style={{ fontSize: "16px" }}>✨</span> AI Verification Analysis
                            </h4>
                            {selectedSubmission.aiConfidence !== null && (
                              <div style={{ fontSize: "13px", color: "var(--text-secondary)", marginBottom: "4px" }}>
                                <strong>Confidence Score:</strong> {selectedSubmission.aiConfidence}%
                              </div>
                            )}
                            {selectedSubmission.aiAnalysis && (
                              <div style={{ fontSize: "13px", color: "var(--text-secondary)", lineHeight: "1.5" }}>
                                <strong>Notes:</strong> {selectedSubmission.aiAnalysis}
                              </div>
                            )}
                          </div>
                        )}
          ...
          {activeTab === "flags" && (
            <>
              ...
              <div className="ar-panel-layout">
                <div className="ar-queue-panel">
                  <div className="ar-queue-header">
                    {flagStatusLabel(flagStatusFilter)} · {filteredFlags.length}
                  </div>
                  {filteredFlags.length === 0 ? (
                    <div className="ar-queue-empty">No flags in this view</div>
                  ) : (
...
export const getServerSideProps = auth0.withPageAuthRequired({
  async getServerSideProps({ req, res }) {
    ...
    // When AUTH0_AUDIENCE is not configured, fall back to DB role
    let isAdmin = auth0Admin;
    if (!isAdmin && !process.env.AUTH0_AUDIENCE && session.user.sub) {
